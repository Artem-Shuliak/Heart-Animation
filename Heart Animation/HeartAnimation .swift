...
//  Heart Animation
//
//  Created by Artem Chouliak on 4/3/21.
//
...
extension UIViewController {
    ...
    func createAnimation(translation: CGPoint, duration: CFTimeInterval) {
        // params
        let duration = duration + Double.random(in: 0...1) * 1
        
        // Create Image
        let imageView = UIImageView(image: UIImage(systemName: "heart.fill"))
        imageView.tintColor = Bool.random() ? .red : .systemPink
    ...
    func customPath(position: CGPoint) -> UIBezierPath {
        let path = UIBezierPath()
        path.move(to: CGPoint(x: position.x, y: position.y))
        
        let endPoint = CGPoint(x: position.x, y: 0)
        
        let randomXShift = 50 + Double.random(in: 0...1) * 400
